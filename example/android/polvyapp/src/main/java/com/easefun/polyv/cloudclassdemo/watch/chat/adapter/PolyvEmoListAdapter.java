...
public class PolyvEmoListAdapter extends PolyvBaseRecyclerViewAdapter {
    ...
    public static class GridSpacingItemDecoration extends RecyclerView.ItemDecoration {
        ...
        public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
            int position = parent.getChildAdapterPosition(view); // item_mine_course position
            int column = position % spanCount; // item_mine_course column
            ...
            if (includeEdge) {
                ...
                outRect.bottom = spacing; // item_mine_course bottom
            } else {
                outRect.left = column * spacing / spanCount; // column * ((1f / spanCount) * spacing)
                outRect.right = spacing - (column + 1) * spacing / spanCount; // spacing - (column + 1) * ((1f /    spanCount) * spacing)
                if (position >= spanCount) {
                    outRect.top = spacing; // item_mine_course top
                }
            }
        }
    }
